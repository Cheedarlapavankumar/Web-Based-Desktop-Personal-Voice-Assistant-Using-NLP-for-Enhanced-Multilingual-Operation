...
const updateProcessingMessage = (Main, message, emojis, replace = false) => {
  if (!processingMessageRow) {
    ...
    processingMessageRow.querySelector(".chat").innerHTML =
      `${message} ${emojis}`;
  } else {
    processingMessageRow.querySelector(".chat").textContent +=
      ` ${message} ${emojis}`;
  }
};
...
async function run(prompt, username) {
  ...
  const chat = model.startChat({
    history: [
      ...
      },
      {
        ...
        parts: [
          {
            text: "My name is Voice Assistant ✨, Trained Engineering Team and developed by Tbh 😎",
          },
        ],
      },
      {
      ...
      },
      {
        ...
        parts: [
          {
            text: "Describe about me and please consider my personal data then answer me",
          },
        ],
      },
      {
        ...
        parts: [
          {
            text: `Yes, i will consider with all about your details, name: ${UserData.name},age:${UserData.age}, gender:${UserData.gender},language;${UserData.language}, about: ${UserData.about}, country: india`,
          },
        ],
      },
    ],
  ...
  if (text.includes("openInBrowser") && text.includes("message")) {
    ...
    return lang !== "en-US"
      ? translator(open.message).then((trans) => {
          return trans;
        })
      : open.message;
  } else {
...
function translator(text, strict = "") {
  ...
  return fetch(
    "https://proxy.cors.sh/https://deep-translator-api.azurewebsites.net/google/",
    {
      method: "POST",
      headers: {
